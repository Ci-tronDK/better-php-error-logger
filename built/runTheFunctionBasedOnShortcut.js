...
exports.runTheFunctionBasedOnShortcut = void 0;
const vscode = __importStar(require("vscode"));
const symbolFinderLoop_1 = require("./symbolFinderLoop");
const getIndentation_1 = require("./getIndentation");
const isBalanced_1 = require("./isBalanced");
const getSelectionType_1 = require("./getSelectionType");
const parsePHPfile_1 = require("./parsePHPfile");
async function runTheFunctionBasedOnShortcut(args, packageJSON) {
    ...
    if (usePHPParserForPositioning) {
        parsedphpFile = (0, parsePHPfile_1.parsePHPfile)(document.fileName, document.getText());
    ...
    editor.edit((editBuilder) => {
        ...
        selections.forEach(selection => {
            ...
            if (usePHPParserForPositioning && parsedphpFile) {
                const selectionType = (0, getSelectionType_1.getSelectionType)(selection, selectedVar, parsedphpFile);
                const selectionTypeToSelectedLine = {
                    ...
                    'function_parameter': (0, symbolFinderLoop_1.symbolFinderLoop)(document, selectedLine - 1, '{'),
                    //First occurence of { after the variable, then first occurence of } after.
                    'switch_variable': (0, symbolFinderLoop_1.symbolFinderLoop)(document, (0, symbolFinderLoop_1.symbolFinderLoop)(document, selectedLine - 1, '{') + 2, '}'),
                    //Find first occurence of ; after an assigned variable.
                    'assigned_variable': (0, symbolFinderLoop_1.symbolFinderLoop)(document, selectedLine, ';'),
                };
                selectedLine = selectionTypeToSelectedLine[selectionType] || selectedLine;
            }
            const indentation = (0, getIndentation_1.getIndentation)(editor, document, selectedLine);
            //Check if the braces in the selected variable are balanced
            if (!(0, isBalanced_1.isBalanced)(selectedVar)) {
            ...
            if (varDumpExportVariable !== 'No var dump or export') {
                ...
                if (!useEchoInstead) {
                    if (dumpOrExport === 'var_export') {
                        ...
                        editBuilder.insert(new vscode.Position(selectedLine + position, 0), `${ob_start}${indentation}${varDumpExportSelectedVar};\n`
                        //+
            ...
            if (beforeEchos) {
                editBuilder.insert(new vscode.Position(selectedLine + position, 0), `${indentation}${beforeEchos}`);
            }
            errorLogs.forEach((errorLog) => {
                ...
                editBuilder.insert(new vscode.Position(selectedLine + position, 0), `${indentation}${errorLogString}${parantheseLeft}${errorLog}${newLine}${parantheseRight}; \n`);
            });
            if (printWithCallStack === 'With call stack as string' || printWithCallStack === 'With call stack as array') {
                editBuilder.insert(new vscode.Position(selectedLine + position, 0), `${indentation}${errorLogString}${parantheseLeft}${print_r_start} (new \\Exception()) -> ${getTrace}()${newLine}${print_r_end}${parantheseRight}; \n`);
            }
            if (afterEchos) {
                editBuilder.insert(new vscode.Position(selectedLine + position, 0), `${indentation}${afterEchos}`);
            }
            if (printCurrentOutputBuffer && varDumpExportVariable !== 'No var dump or export' && dumpOrExport === 'var_dump') {
                editBuilder.insert(new vscode.Position(selectedLine + position, 0), `echo ${outbutbufferVariable}; \n`);
            }
        });
    });
}
exports.runTheFunctionBasedOnShortcut = runTheFunctionBasedOnShortcut;
//# sourceMappingURL=runTheFunctionBasedOnShortcut.js.map